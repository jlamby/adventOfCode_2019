...
class Tree : 
    ...
    def improvedSearch(self, node, data, path) :
        #print("Searching for {} in node {}".format(data, node))
        for n in node.nodes :
            if (n.data == data) : 
                path.append(node)
            ...
            foundNode = self.improvedSearch(n, data, path)
...
tree.improvedSearch(rootNode, 'YOU', youNodePath)
tree.improvedSearch(rootNode, 'SAN', sanNodePath)

youSteps = set(node.data for node in youNodePath)
sanSteps = set(node.data for node in sanNodePath)

diff = youSteps ^ sanSteps
print(len(diff))