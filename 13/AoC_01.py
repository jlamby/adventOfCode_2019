...
class Tile :
    def __init__(self, x, y, tileTypeId) :
        self.x = x
        self.y = y
        self.tileType = TileType(tileTypeId)

    def __str__(self) : return f'({self.x}, {self.y}) {self.tileType.name}'
...
class TileType(Enum) :
    ...
    def retrieveGlyph(self) :
        if self == TileType.BALL  : return 'B' 
        if self == TileType.BLOCK : return '▒'
        if self == TileType.EMPTY : return ' '
        if self == TileType.PADDLE: return 'P'
        if self == TileType.WALL  : return '█'

class JoystickOrientation(Enum) : 
    LEFT = -1
    NEUTRAL = 0
    RIGHT = 1

def handleOutputDatas(outputData) : 
    global buffer

    if len(buffer) == 2 : 
        if buffer[0] == -1 and buffer[1] == 0 :
            handleScore(outputData)
        else : 
            handleTile(outputData)
        resetBuffer()
    else :
...
def handleTile(data) :
    global buffer, tileSet
    
    tile = Tile(buffer[0], buffer[1], data)
    tileSet[tile.y][tile.x] = tile

def resetBuffer() : global buffer; buffer = []

def computePaddleMove() :
    global tileSet

    paddle = findTile(tileSet, TileType.PADDLE)
    ball = findTile(tileSet, TileType.BALL)

    if paddle.x < ball.x : return JoystickOrientation.RIGHT
    if paddle.x > ball.x : return JoystickOrientation.LEFT

    return JoystickOrientation.NEUTRAL

def findTile(tileSet, tileType) :
    for row in tileSet :
        for element in row :
            if element != None and element.tileType == tileType :
                return element
    return None
...
def draw() :
    ...
    for row in tileSet :
        for element in row : 
            ...
            if element != None : 
                print(tile.tileType.retrieveGlyph(), end='')
        if row[0] != None :
            print('')
...
buffer = []
tileSet = [[None for i in range(99)] for j in range(99)]
score = 0 
